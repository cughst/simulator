...
void PeriodicOutput()
{
    std::cout << "Data output at " << Simulator::Now().GetNanoSeconds() << " ns" << std::endl;

    // 输出指针变量的值或相关状态
    std::cout << "egress qlen \t" << globalSw->qlen_value << std::endl;
	std::cout << "output rate \t" << globalSw->tx_rate << std::endl;
	std::cout << "ecn rate \t" << globalSw->tx_rate_m << std::endl;
	std::cout << "kmin \t" << globalSw->min_thresh<< std::endl;
	std::cout << "kmax \t" << globalSw->max_thresh << std::endl;
	std::cout << "pmax \t" << globalSw->pmax_thresh<< std::endl;

    // 重新调度下一次事件
    Simulator::Schedule(NanoSeconds(500), &PeriodicOutput);
}
*/
// Global pointer for getting switch node data
Ptr<SwitchNode> globalSw;
...
struct sEcnRlEnv
{
	//u_int32_t qlen_array[257][8];
    //double link_rate[257];
...
public:
    ...
    //need a function to set ecn 
	void ScheduleNextStateRead ();
 	bool m_started{false};
  	Time m_timeStep{NanoSeconds(1000)};
protected:
  ...
  uint32_t m_new_max;
  double m_new_pmax;
//   Ptr<SwitchNode> m_switchnode = globalSw;
...
void
EcnRlEnv::ScheduleNextStateRead(){
	
	std::cout<<"get state"<<"\n";
	
	auto envi = EnvSetterCond();
  	envi->envType = 1;
  	envi->simTime_us = Simulator::Now().GetNanoSeconds();
  	envi->egressqlen = globalSw->qlen_value;
  	envi->linkrate = globalSw->tx_rate;
  	envi->ecnlinkrate = globalSw->tx_rate_m;
  	envi->ecnmin = globalSw->min_thresh;
 	envi->ecnmax = globalSw->max_thresh;
  	envi->ecnpmax = globalSw->pmax_thresh;

  	std::cerr<<(u_int64_t)(Simulator::Now().GetNanoSeconds())<<" "<<envi->egressqlen<<" "<<envi->linkrate<<" "<<envi->ecnlinkrate<<
 	 " "<<envi->ecnmin<<" "<<envi->ecnmax<<" "<<envi->ecnpmax<<" "<<std::endl;
  	
        SetCompleted();
  	auto act = ActionGetterCond();
 	m_new_mix = act->newecnmin;
 	m_new_max = act->newecnmax;
  	m_new_pmax = act->newecnpmax;
  	GetCompleted();
  	
  	std::cerr<<(u_int64_t)(Simulator::Now().GetNanoSeconds())<<" "<<m_new_mix<<" "<<m_new_max<<" "<<m_new_pmax<<" "<<std::endl;
  	Simulator::Schedule(m_timeStep, &EcnRlEnv::ScheduleNextStateRead, this);
	for (u_int16_t i = 0; i < globalSw->m_mmu->pCnt; i++)
	{
		globalSw->m_mmu->ConfigEcn(i,m_new_mix,m_new_max,m_new_pmax);
	}	
}
/////////////////////////////////////////////////////////////////////////////
...
void qp_finish(FILE* fout, Ptr<RdmaQueuePair> q){
	...
	uint64_t base_rtt = pairRtt[sid][did], b = pairBw[sid][did];//计算rtt和bw
...
int main(int argc, char *argv[])
{
	...
	int memblock_key = 2330;        ///< memory block key, need to keep the same in the python script
	...
	for (uint32_t i = 0; i < 7; ++i) {
		...
		if (!node->GetObject<MobilityModel>()) {
				...
				mobility->SetPosition(Vector(i*10, 70,0));
		}
	}
	for (uint32_t i = 7; i < 8; ++i) {
		...
		if (!node->GetObject<MobilityModel>()) {
				...
				mobility->SetPosition(Vector((i-5)*10, 40,0));
		}
	}  
	...
	// AnimationInterface anim("test_cc.xml");

	Simulator::Stop(Seconds(simulator_stop_time));

	// Simulator::Schedule(Seconds(2.0), &PeriodicOutput);
	
	//Time delay = ns3::Seconds (2.0);
	///////////////////////////////////////////////////////
	//Set the start time and schedule according to time steps
	///////////////////////////////////////////////////////
	Simulator::Schedule(Seconds(2.0),&EcnRlEnv::ScheduleNextStateRead,&ecnrlenv);
