...
public:
	...
	void ScheduleNextStateRead ();
 	bool m_started{false};
  	Time m_timeStep{NanoSeconds(2000)};
protected:
...
EcnRlEnv::ScheduleNextStateRead(){
	...
	globalSw->duration = 2000;
  	env->envType = 1;
  	env->simTime_us = Simulator::Now().GetNanoSeconds();
  	env->egressqlen = globalSw->qlen_value;
  	env->linkrate = globalSw->tx_rate;
  	env->ecnlinkrate = globalSw->tx_rate_m;
  	env->ecnmin = globalSw->min_thresh;
 	env->ecnmax = globalSw->max_thresh;
  	env->ecnpmax = globalSw->pmax_thresh;
  	//std::cerr<<(u_int64_t)(Simulator::Now().GetNanoSeconds())<<" "<<env->egressqlen<<" "<<env->linkrate<<" "<<env->ecnlinkrate<<
 	//" "<<env->ecnmin<<" "<<env->ecnmax<<" "<<env->ecnpmax<<" "<<std::endl;
  	//stop 
    SetCompleted();
	if (globalSw->qlen_value == 60 ) {
        Simulator::Stop();
    }
...
void qp_finish(FILE* fout, Ptr<RdmaQueuePair> q){
    ...
    // Check if all QPs have finished and end the simulation if so
    if (*pActiveQPs == 0) {
        Simulator::Stop();
    }*/
}
...
int main(int argc, char *argv[])
{
	...
	qbb.EnablePcapAll("sim_ecn",true);
	internet.EnablePcapIpv4All("sim_e");
	Simulator::Run();
