...
namespace ns3 {
	...
	void SwitchMmu::UpdateEgressAdmission(uint32_t port, uint32_t qIndex, uint32_t psize){
		...
		//std::cout<<"port "<<port<<"egress qlen"<<egress_bytes[port][qIndex]<<"\n";
	}
	//2024 3 3 添加
	void SwitchMmu::EgressQlen(uint32_t port, uint32_t qIndex, uint32_t psize){
		// u_int64_t sum = 0;
		u_int32_t (*ptr)[8] = egress_bytes;
		//u_int32_t avg= 0;
		for (u_int32_t i = 0; i < pCnt; i++){
			for (u_int32_t j = 0; j < qCnt; j++){
				if(egress_bytes[i][j] != 0){
					printf("%d ", *(*(ptr + i) + j));
				}
			}
		}
