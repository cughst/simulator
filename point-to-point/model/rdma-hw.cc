...
TypeId RdmaHw::GetTypeId (void)
{
	static TypeId tid = TypeId ("ns3::RdmaHw")
		...
		.AddAttribute("MinRate",
				...
				MakeDataRateChecker())
		// .AddAttribute("currentRate",
		// 		"current rate of flow",
		// 		DataRateValue(DataRate("0Mb/s")),
		// 		MakeDataRateAccessor(&RdmaHw::m_c),
		// 		MakeDataRateChecker())		
		.AddAttribute("Mtu",
				"Mtu.",
				UintegerValue(1000),
...
int RdmaHw::ReceiveAck(Ptr<Packet> p, CustomHeader &ch){
	...
	// handle cnp
	// std::cout<<"ack check cnp "<<"\n";
	if (cnp){
		// add
		// std::cout<<"cnp==1 "<<"\n";
		if (m_cc_mode == 1 || m_cc_mode ==  0){ // mlx version
			cnp_received_mlx(qp);
...
int RdmaHw::Receive(Ptr<Packet> p, CustomHeader &ch){
	if (ch.l3Prot == 0x11){ // UDP  17
		ReceiveUdp(p, ch);
	}else if (ch.l3Prot == 0xFF){ // CNP   255
		ReceiveCnp(p, ch);
	}else if (ch.l3Prot == 0xFD){ // NACK   253
		ReceiveAck(p, ch);
	}else if (ch.l3Prot == 0xFC){ // ACK  252
		u_int32_t aa  =ReceiveAck(p, ch);
		return aa;
	}
	return 0;
}
...
void RdmaHw::cnp_received_mlx(Ptr<RdmaQueuePair> q){
	...
	// std::cout<<"current rate "<<q->m_rate<<"\n";
	m_c = q->m_rate.GetBitRate();
	m_current = q->m_rate.GetBitRate();
	// std::cout<<"current rate "<<m_c<<"bps \n";
	// std::cout<<"check "<<"\n";
...
void RdmaHw::CheckRateDecreaseMlx(Ptr<RdmaQueuePair> q){
	...
	if (q->mlx.m_decrease_cnp_arrived){
		...
		// std::cout<<q->sip.Get()<<"current rate "<<q->m_rate<<"\n";
		// decrease rate processing
